...
def invite(request, success_url=None,
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if remaining_invitations < 0 or remaining_invitations >= len(emails):
                for email in emails:
                    invitation = InvitationKey.objects.create_invitation(request.user, email)
                    invitation.send_to(email, email_subject_template, email_template, message)
