...
class InvitationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.saved_invite_multiple_emails = settings.INVITE_MULTIPLE_EMAILS
        settings.INVITE_MULTIPLE_EMAILS = True
    ...
    def tearDown(self):
        ...
        settings.INVITE_MULTIPLE_EMAILS = self.saved_invite_multiple_emails
...
class InvitationModelTests(InvitationTestCase):
    ...
    def test_invitations_remaining(self):
        ...
        # Use some invites
        InvitationKey.objects.create_invitation(
            user=user, email=self.sample_registration_data['email'])
        InvitationKey.objects.create_invitation(
            user=user, email=self.sample_registration_data['email'])

        # After using some, amount remaining is decreased
        used = InvitationKey.objects.filter(from_user=user).count()
...
class InvitationViewTests(InvitationTestCase):
    ...
    def test_invitation_view(self):
        ...
        self.assertTrue(response.context['form'])
        self.assertEqual(len(mail.outbox), remaining_invitations)

    def test_multi_invitation_view(self):
        """
        Test that the invitation view rejects invalid submissions,
        and creates a new key and redirects after a valid submission.
        ...
        # Empty email data fails.
        response = self.client.post(reverse('invitation_bulk_invite'),
                                    data={'emails': ''})
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.context['form'])
        self.assertTrue(response.context['form'].errors)

        # Invalid single email data fails.
        response = self.client.post(reverse('invitation_bulk_invite'),
        ...
        # Invalid multiple email data fails.
        response = self.client.post(reverse('invitation_bulk_invite'),
                                    data={'emails': 'example.com, example2.com'})
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.context['form'])
        self.assertTrue(response.context['form'].errors)
        ...
        # Valid email data succeeds.
        response = self.client.post(reverse('invitation_bulk_invite'),
                                    data={'emails': 'foo@example.com'})
        self.assertRedirect(response, 'invitation_complete')
        self.assertEqual(InvitationKey.objects.count(), 3)
        self.assertEqual(InvitationKey.objects.remaining_invitations_for_user(self.sample_user), remaining_invitations - 1)
        self.assertEqual(len(mail.outbox), 1)

        # Valid multiple email data succeeds.
        response = self.client.post(
            reverse('invitation_bulk_invite'),
            data={'emails': 'foo@example.com, bar@example.com; c@d.com\ne@f.com'})
        self.assertRedirect(response, 'invitation_complete')
        self.assertEqual(InvitationKey.objects.count(), 7)
        self.assertEqual(InvitationKey.objects.remaining_invitations_for_user(self.sample_user),
                         remaining_invitations - 5)
        self.assertEqual(len(mail.outbox), 5)

        # Once remaining invitations exhausted, you fail again.
        while InvitationKey.objects.remaining_invitations_for_user(self.sample_user) > 0:
            self.client.post(reverse('invitation_bulk_invite'),
                             data={'emails': 'foo@example.com'})
        self.assertEqual(InvitationKey.objects.remaining_invitations_for_user(self.sample_user), 0)
        response = self.client.post(reverse('invitation_bulk_invite'),
                                    data={'emails': 'foo@example.com'})
...
class InviteMultipleEmailsOffTests(InvitationTestCase):
    """
    Tests for the case where INVITE_MULTIPLE_EMAILS=False

    (The test cases above generally assume that INVITE_MULTIPLE_EMAILS=True)
    ...
    def setUp(self):
        super(InviteMultipleEmailsOffTests, self).setUp()
        settings.INVITE_MULTIPLE_EMAILS = False


    def test_invitation_view(self):
        """
        Test that the invitation view creates a new key and redirects after a valid submission.
    ...
    def test_multi_invitation_view(self):
        ...
        # Valid single email data succeeds.
        response = self.client.post(reverse('invitation_invite'),
                                    data={'email': 'foo@example.com'})
        self.assertRedirect(response, 'invitation_complete')
        self.assertEqual(InvitationKey.objects.count(), 3)
        ...
        # Valid multiple email data fails.
        response = self.client.post(
            reverse('invitation_bulk_invite'),
            data={'emails': 'foo@example.com, bar@example.com; c@d.com\ne@f.com'})
        self.assertEqual(response.status_code, 200)
...
class InviteUnlimitedModeTests(InvitationTestCase):
    """
    Tests for teh case where INVITATIONS_PER_USER is set to -1 indicating that
    each user gets an unlimited number of invitations.
    ...
    def setUp(self):
        super(InviteUnlimitedModeTests, self).setUp()
        settings.INVITATIONS_PER_USER = -1
    ...
    def test_invitations_remaining(self):
        ...
        # After using some, amount remaining stays the same
        used = InvitationKey.objects.filter(from_user=user).count()
        self.assertEqual(used, 2)
        remaining = remaining_invites(user)
        self.assertEqual(remaining, settings.INVITATIONS_PER_USER)
    ...
    def test_invitation_view(self):
        """
        Test that the invitation view successfully creates a new key
        ...
        self.assertRedirect(response, 'invitation_complete')
        self.assertEqual(InvitationKey.objects.count(), 3)
        self.assertEqual(InvitationKey.objects.remaining_invitations_for_user(self.sample_user), remaining_invitations)
        self.assertEqual(len(mail.outbox), 1)

    def test_multi_invitation_view(self):
        """
        Test that the bulk invitation view successfully creates keys
        after a valid submission.
        """
        # You need to be logged in to send an invite.
        response = self.client.login(username='alice', password='secret')
        ...
        # Valid email data succeeds.
        response = self.client.post(reverse('invitation_bulk_invite'),
                                    data={'emails': 'foo@example.com, bar@example.com'})
        self.assertRedirect(response, 'invitation_complete')
        self.assertEqual(InvitationKey.objects.count(), 4)
        self.assertEqual(InvitationKey.objects.remaining_invitations_for_user(self.sample_user), remaining_invitations)
